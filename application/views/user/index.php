...
<div id="global">
    ...
    <div class="header">
        ...
        <div class="wide_bg_m">
            <div class="m_header">
                <div class="logo fl"><a href="/<?= lang('main_lang') ?>"><img src="/media/css/user/img/alafa.png" height="50"/> <span class="logo_h">АЛАФА</span></a>
                </div>
                <div class="menu fl">
                    <ul>
                        <?php $count_pages = count($pages);
                            for($i = 0; $i < $count_pages; $i++){
                                if($pages[$i]['show_top'] == '1'){
                                    ...
                                    if($pages[$i]['module'] == 'gallery'){
                                        $url = lang('main_lang').'/gallery';
                                    }
                                    if($pages[$i]['module'] == 'documents'){
                                        $url = lang('main_lang').'/documents';
                                    }
                                    if($pages[$i]['module'] == 'reviews'){
                                        $url = lang('main_lang').'/reviews';
                                    }
                                    if($pages[$i]['module'] == 'news'){
                                        $url = lang('main_lang').'/news';
                                    }
                                    ?>
                                    <li><a href="<?php echo base_url(); ?><?= $url ?>"
                                           class="menu <?php if(isset($current_page) && $current_page == $pages[$i]['id']){
                                               echo 'active';
                                           } ?>"><?= $pages[$i]['menu_name'] ?></a></li>
                                <?php }
                            } ?>
                    </ul>
                </div><!--menu-->
                ...
                <div class="language fr">
                    ...
                    <ul> <?php $qqq = substr($_SERVER['REQUEST_URI'], 3);
                            $lang = $this->uri->segment(1);
                            if($lang == ''){
                                $lang = 'ru';
                            } ?>
                        <li><a main="ua" href="<?= base_url() ?>ua<?= $qqq ?>" <?php if($lang == 'ua'){
                                echo 'class="actual"';
                            } ?> title="Перейти на Українську">укр</a></li>
                        <li><a main="ru" href="<?= base_url() ?>ru<?= $qqq ?>" <?php if($lang == 'ru'){
                                echo 'class="actual"';
                            } ?> title="Перейти на русский язык">рус</a></li>
                        <!-- <li><a  main="en"  href="<?= base_url() ?>en<?= $qqq ?>" <?php if($lang == 'en'){
                            echo 'class="actual"';
                        } ?> title="Go to English" >eng</a></li> -->
                    </ul>
                </div><!--lang-->
                <div class="logo_fond fr">
                    <noindex><img src="/media/css/user/img/logo_fond_md_<?= lang('main_lang') ?>.png" height="44"/></noindex>
                </div>
    ...
    <div class="clear"></div>
    <?php if(isset($index)){ ?>
        <div class="slider_12"></div><?php } ?>


    <?php if(isset($index) && (!isset($user['email']) || !isset($user['user_id']))){ //echo $user['email']."  === ".$user['user_id']; ?>
        <div class="wide_green_bg">
            <div class="m_video">
                <div class="video"><?php if(isset($blocks[7]) && $blocks[7]['text'] != '' && $blocks[7]['text'] != '&nbsp;'){
                        echo $blocks[7]['text'];
                    } ?></div>
                <div class="opis"><?php if(isset($blocks[8]) && $blocks[8]['text'] != '' && $blocks[8]['text'] != '&nbsp;'){
                        echo $blocks[8]['text'];
                    } ?></div>
                <div class="clear"></div>
            </div> <!--mvideo-->
        ...
        <div class="m_red">
            <div class="do_some_buttons">
                <?php
                    ...
                    <?php }else{ ?>
                        <div class="go_to_login_form"
                             onClick="window.location='/<?= lang('main_lang') ?>/logout' "><?= lang('main_user_logout') ?></div>
                        <div class="go_to_login_form"
                             onClick="window.location='/<?= lang('main_lang') ?>/user'  "><?= lang('main_user_account') ?></div>
                        <div class="clear"></div>
                    <?php } ?>
            ...
            <div class="text">
                <div class="some_te login_text"><?php if(isset($blocks[0]) && $blocks[0]['text'] != '' && $blocks[0]['text'] != '&nbsp;'){
                        echo $blocks[0]['text'];
                    } ?></div>
                <div class="some_te register_text"><?php if(isset($blocks[1]) && $blocks[1]['text'] != '' && $blocks[1]['text'] != '&nbsp;'){
                        echo $blocks[1]['text'];
                    } ?></div>
                <div class="some_te forget_text"><?php if(isset($blocks[2]) && $blocks[2]['text'] != '' && $blocks[2]['text'] != '&nbsp;'){
                        echo $blocks[2]['text'];
                    } ?></div>
            </div><!--text-->
            ...
            <div class="fullform">
                <div class="forma login_form">
                    <fieldset>
                        ...
                        <form class="login_form" method="post" action="<?php echo base_url(); ?>login/loginSubmit" enctype="multipart/form-data"
                              name="myform_login" id="myform_login">
                ...
                <div class="forma register_form">
                    <?php
                        ...
                        if(isset($referal_code)){
                            $referal_id = $referal_code;
                        }

                    ?>
                    <fieldset>
                        <form class="register_form" method="post" action="<?php echo base_url(); ?>login/addregister" enctype="multipart/form-data"
                              name="myform_regist" id="myform_regist">
                            <div align="center"><span><?= lang('main_user_reg_and_take_code1') ?></span></div>

                            <div class="for_fiz_l" onClick="action_register_form_fiziki_on();"><span
                                    class="activ_type"><?= lang('main_user_reg_fizlic') ?></span></div>
                            <!--<div class="for_ur_l" onClick="action_register_form_uriki_on();"><span ><?= lang('main_user_reg_urlic') ?></span></div>-->

                            <input name="referal" type="text" value="<?php if(isset($referal_id) && $referal_id != ''){
                                echo $referal_id;
                                $is_ref = 1;
                            } ?>" <?php if(isset($is_ref)){
                                echo " readonly ";
                            } ?> placeholder="<?= lang('main_user_code_partnera') ?>"/>
                            <input name="name" type="text" value="" placeholder="<?= lang('main_form_name') ?>" required/>

                            <input name="email" type="text" placeholder="Email" required/>
                            <input name="pword" type="password" placeholder="<?= lang('main_user_parol') ?>" title="<?= lang('main_user_parol') ?>"
                                   required/>
                            <input name="re_pword" type="password" placeholder="<?= lang('main_user_povtorite_parol') ?>"
                                   title="<?= lang('main_user_povtorite_parol') ?>" required/>
                ...
                <div class="forma forget_form">
                    <fieldset>
                        <form class="forget_form" method="post" action="<?php echo base_url(); ?>login/remind_password" enctype="multipart/form-data"
                              name="myform_forget" id="myform_forget">
                            ...
                            <div align="center"><input class="bottom" name="btn_fg" type="submit" value="<?= lang('main_user_vislat_novy_parol') ?>"/>
                            </div>
                        </form>
                    </fieldset>
                    <div class="hrefs">
    ...
    <div class="howitworks">
        <?php if(isset($index)){ ?>
            ...
            <div class="m_howit">
                <!--КАК ЭТО РАБОТАЕТ-->
                <?php if(isset($blocks[9]) && $blocks[9]['text'] != '' && $blocks[9]['text'] != '&nbsp;'){
                    echo $blocks[9]['text'];
                } ?>
            </div><!--m_howit-->
        ...
        <div class="prom_box_block">
            <!--РЕКЛАМА-->
            <?php if(isset($blocks[5]) && $blocks[5] != '' && $blocks[5]['text'] != '' && $blocks[5]['text'] != '&nbsp;'){
                echo $blocks[5]['text'];
            } ?>
            <!--<p>Рекламное место</p>
    ...
    <div class="footer">
        <div class="m_footer">
            ...
            <div class="media">
                ...
                <a href="https://www.youtube.com/channel/UCX_ltIoiwhacMtlCcwQRoNw"><img src="/media/css/user/img/icon_youtube.png" width="34"
                                                                                        height="34"/></a>
                <a href="https://new.vk.com/alafadobro"><img src="/media/css/user/img/icon_vk.png" width="34" height="34"/></a>
                <a href="http://wallet.advcash.com/referral/3b959503-84d7-47cd-abb4-c6d28494274b" target="_blank"><img
                        src="/media/images/promo_ru/paylogo.gif" height="32"/></a>
            </div>
            <div class="clear"></div>
        </div><!--m_footer-->
        <div class="clear"></div>
...
<?php
    ...
    $ip = ["109.86.137.253", "93.73.54.100"];
