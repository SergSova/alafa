...
//****************************************	
 	function do_delete_callback(id_product){	 
                $.ajax({  
                    type: "POST",  
                    url:   "/manage_statistic/delete_callback/"+id_product,
					cache: false, 
    ...
    <tr> 
             ...
             <?php $count = 0; foreach($list['list'] as $prds): $count++;?>
              ...
              <div class="set_comment_list_popular list_comment_<?=$prds['id']?>"  onclick="change_list_comment_do(<?=$prds['id']?>);"> 
                        ...
                        <button onclick="save_list_callback(<?=$prds['id']?>); return false;" style="color: #333; font-size:8px;">Применить</button> 
                        </div>
