...
function confirmChangeVisibleOff() {
	if (confirm("Вы уверены, что хотите сделать запись невидимой?")) {
		return true;
	} else {
		return false;
	}
...
function confirmChangeVisibleOn() {
	if (confirm("Вы уверены, что хотите сделать запись видимой пользователю?")) {
		return true;
	} else {
		return false;
	}
...
$(document).ready(function() { 
 ...
 $.ajax({
		...
		beforeSend: function () {
            ...
            type:       "POST",
            url:        "<?php echo base_url();?>manage_news/update_reviews_number",
...
<div class="td-caption-top"><a href="<?php echo base_url();?>manage_news/add_review">
  <img src="<?php  echo base_url();?>media/images/add_material.png" alt="Добавить отзыв " title="Добавить отзыв" align="top" height="25" border="0"/></a>
   Отзывы  </div> 
   
        <?php  if (empty($reviews['list'])) { ?>
        <div class="list-alert"> В этом списке нет отзывов. Вы их можете добавить нажав плюс</div>
        <?php	} ?>
  <?php if (!empty($reviews['list'])): ?>
<!-- LIST Begin -->
        ...
        <td  class="td-caption">Заголовок </td>
   ...
   <table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <td width="35" valign="middle"><img src="<?php echo base_url();?>media/images/arrow-list-move.png" alt="move" width="20" height="20" class="handle" align="left" /></td>
            ...
            <td width="35" class="column" align="center">
                <?php	  if($a['visible'] == '1'){  ?>
                         <a href="<?php echo base_url();?>manage_news/edit_review_visible/<?php echo $a['id']; ?>/<?php echo $a['visible']; ?>" onclick="return confirmChangeVisibleOff();">
                         <img src="<?php  echo base_url();?>media/images/globe_32.png" height="20"  alt="Видимая. Сделать невидимой" title="Видимая. Сделать невидимой"  border="0"/></a><?php } 
                            if($a['visible'] == '0') {?>
                         <a href="<?php echo base_url();?>manage_news/edit_review_visible/<?php echo $a['id']; ?>/<?php echo $a['visible']; ?>" onclick="return confirmChangeVisibleOn();">
            ...
            <a href="<?php echo base_url();?>manage_news/edit_review/<?php  echo $a['id'];?>">  
             <img src="<?php echo base_url();?>media/images/action-edit.png" height="20" border="0" title='Редактировать' />
              </a>
            </td>
            <td width="35" class="column">
            <a href="<?php echo base_url();?>manage_news/delete_review/<?php echo $a['id']; ?>"  onclick="return confirmDelete();">
